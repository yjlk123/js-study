...
getGeoJson(adcode).then(data => {
  geoJson = data // 获取到阿里地图
  getMapData()
}) 
}


// 获取到阿里地图，这里我封装了下，直接可以拿过来用
...
function initEchartMap(mapData, sum, pointData) {
  ...
  var option = {
      ...
      baseOption: {
          ...
          geo: {
              map: parentInfo.length === 1 ? 'china' : 'map', // 使用的地图方式是第一种：script 标签引入的 js 文件，引入后会自动注册地图名字和数据。 series[0].map 要对应这个值
              zoom: 1.1,
              roam: true,
              left:'10%',
              top:'15%',
